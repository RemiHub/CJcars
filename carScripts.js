...
window.addEventListener(
  "click",
  (e) => (e.target == modal ? (modal.style.display = "none") : false)
...
function filterCarList() {
  ...
  searchBar.addEventListener("keyup", (e) => {
    ...
    //                                  const gridItem = document.querySelectorAll('#grid-container');

    // for(var i = 0; i < gridItem.length; i++){
    //     const carGrid = gridItem[i]
    //     // console.log(carGrid)

    Array.from(carList).forEach((car) => {
      const make = car.textContent;

      if (make.toLowerCase().indexOf(term) > -1) {
        car.style.display = "";
      } else {
        car.style.display = "none";
        car.parentElement.parentElement.parentElement.style.display = "none";
        console.log(
          (car.parentElement.parentElement.parentElement.style.display = "none")
        );
      }

      // let emptyCarGrid = car.parentElement.parentElement.parentElement.style.display === 'none';

      // for(var i =0; i < emptyCarGrid.length; i++){
      //     console.log(emptyCarGrid[i])
      // }
